...
try:
    ...
    df = spark.read.csv(data_path, header=True, inferSchema=True, sep=",")
    print("⚠️ Switched to comma-separated CSV reading.")
...
if "Year_Birth" in df_clean.columns:
    ...
    print("⚠️ 'Year_Birth' column not found — skipping Age calculation.")
...
if all(colname in df_clean.columns for colname in ["Income", "MntWines", "MntFruits"]):
    ...
    print("⚠️ Some columns missing for filtering (check dataset header).")
